---
name: typed-authoring-vocabulary
description: How to author against canon's typed vocabulary (S10) — declare a new directive or enum in a canon/vocab/<id> plugin, author a typed task atom with a policy-resolved evidence requirement, author a vocabulary-defined handoff body, and when to run canon context first. Use when adding a directive/enum, writing a `::task` or `::handoff-<domain>` atom, or wiring a consumer repo's vocabulary plugin.
---
