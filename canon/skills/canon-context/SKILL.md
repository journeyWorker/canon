...
- **A capability query, never validation:** `canon context` ALWAYS exits
  `0` with the full surface — even when `canon fmt --check` or
...
- `--repo` resolves through the same nearest-`canon.yaml`-ancestor walk
  `canon fmt`/`canon gate`/`canon retrieve` use: omitted or `.` walks up
...
AND `canon fmt`'s diagnostics from one edit site, never two.

## Reading an "expected one of: …" validator error

When `canon fmt --check` rejects an out-of-domain enum value it emits:
...
- `canon fmt --check`'s full violation taxonomy — see `canon-fmt`;
