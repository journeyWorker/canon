...
> How to author against canon's typed vocabulary — declare a directive or enum in a canon/vocab/<id> plugin, author a typed `::task` atom with a policy-resolved evidence requirement, author a `::handoff-<domain>` body, and why to run canon context first. Use when adding a directive/enum, or writing a typed task or handoff atom.
